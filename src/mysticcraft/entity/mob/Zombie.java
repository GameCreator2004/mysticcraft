...
public class Zombie extends Mob {
	...
	public void render(Screen screen) {
		...
		if (lvl == 2)
			col = Color.get(-1, 100, 522, 050, -1); // lvl 2 colour pink
		if (lvl == 3)
			col = Color.get(-1, 111, 444, 050, -1); // lvl 3 light gray
		if (lvl == 4)
			col = Color.get(-1, 000, 111, 020, -1); // lvl 4 dark grey
	...
	protected void die() {
		...
		// SoundUtils.play(Sound.zombieDeath);
		int count = random.nextInt(2) + 1; // Random amount of cloth to drop
											// from 1 to 2
		for (int i = 0; i < count; i++) { // loops through the count

			level.dropItem(x + random.nextInt(11) - 5, y + random.nextInt(11) - 5, new ResourceItem(Resource.cloth, 1), new ResourceItem(Resource.bone, 1), new ResourceItem(Resource.rottenFlesh, 1));

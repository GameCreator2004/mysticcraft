...
public class ItemEntity extends Entity {
	...
	public ItemEntity(Item item, int x, int y) {
		...
		za = random.nextFloat() * 0.7 + 1; // random direction for z
											// acceleration

		lifeTime = 60 * 10 + random.nextInt(60); // sets the lifetime of the
													// item. min = 600 ticks,
													// max = 629 ticks.
	}
	...
	public void tick() {
		...
		if (time >= lifeTime) { // if the time is larger or equal to lifeTime
								// then...
		...
		xx += gotx - expectedx; // new xx position based on the difference
								// between gotx and expectedx
		yy += goty - expectedy; // new yy position based on the difference
								// between goty and expectedy
	}
	...
	public void render(Screen screen) {
			...
			if (time >= lifeTime - 6 * 20) { // if time is larger or equal to
												// lifeTime - 6 * 20 then...
				if (time / 6 % 2 == 0)
					return; // if the remainder of (time/6)/2 = 0 then skip the
							// rest
							// of the code.
			}
		
		screen.render(x - 4, y - 4, item.getSprite(), Color.get(-1, 0, 0, 0, -1), 0); // render
																					// the
																					// shadow
		screen.render(x - 4, y - 4 - (int) (zz), item.getSprite(), item.getColor(), 0); // render
																						// the
																						// item
																						// based
																						// on
	...
	public void touchedBy(Entity entity) {
		if (time > 30)
			entity.touchItem(this); // if time is above 30, it will call the
									// touchItem() method in an entity
									// (Player.java)
	}

...
public class ToolRecipe extends Recipe {
	...
	/*
	 * Blue text adds description notes to a variable. Hover your mouse over the
	 * blue word 'type' or 'level'
	 */
	...
	/**
	 * Adds a recipe to add a tool, input a ToolType and it's level. Example:
	 * ToolRecipe(ToolType.sword , 2) will create a iron sword.
	 */
	public ToolRecipe(ToolType type, int level) {
		super(new ToolItem(type, level)); // this goes through Recipe.java to be
											// put on a list.
	...
	public void craft(Player player) {
		player.inventory.add(0, new ToolItem(type, level)); // adds the tool
															// into your
															// inventory
		Sys.println("Crafted " + new ToolItem(type, level).getName(), System.out);
	}
}

...
public class Menu {
	...
	/**
	 * renders items in a list, used for the inventory, crafting, and chest
	 * menus
	 */
	public void renderItemList(Screen screen, int xo, int yo, int x1, int y1, List<? extends ListItem> listItems, int selected) {
		boolean renderCursor = true;// Renders the ">" "<" arrows between a
									// name.
		if (selected < 0) {
			selected = -selected - 1; // If the selected is smaller than 0, then
										// it will add one.
		...
		if (i1 > h)
			i1 = h; // If the end variable is larger than the height variable,
					// then end variable will equal height variable.
		int io = selected - h / 2; // Middle of the list, (selected item). For
									// scrolling effect
		if (io > listItems.size() - h)
			io = listItems.size() - h; // if the middle is coming near the
										// bottom, then the selected will
										// change.
		if (io < 0)
			io = 0; // if the middle is coming near the top, then the selected
					// will change

		for (int i = i0; i < i1; i++) {
			listItems.get(i + io).renderInventory(screen, (1 + xo) * 8, (i + 1 + yo) * 8); // this
																							// will
																							// render
																							// all
																							// the
		...
		if (renderCursor) {
			int yy = selected + 1 - io + yo; // the y position of the currently
												// selected item
			Font.draw(">", screen, (xo + 0) * 8, yy * 8, Color.get(5, 555, 555, 555, -1)); // draws
																						// the
																						// left
																						// cursor
																						// next
																						...
																						// the
																						// name
			Font.draw("<", screen, (xo + w) * 8, yy * 8, Color.get(5, 555, 555, 555, -1)); // draws
																						// the
																						// right
																						// cursor
																						// next

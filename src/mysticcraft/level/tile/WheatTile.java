...
public class WheatTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		int col = Color.get(level.dirtColor - 121, level.dirtColor - 11, level.dirtColor, 50, -1); // gets
																								// the
																								// color
																								// of
																								// the
		...
		if (icon >= 3) { // if the icon is larger or equal to 3.
			col = Color.get(level.dirtColor - 121, level.dirtColor - 11, 50 + (icon) * 100, 40 + (icon - 3) * 2 * 100, -1); // adds
																														// more
																														// color
																														// based
																														// on
			...
			if (age == 50) { // if the age is equal to 50 then...
				col = Color.get(0, 0, 50 + (icon) * 100, 40 + (icon - 3) * 2 * 100, -1); // changes
																						// the
																						// color
																						// again
																						// (for
		...
		screen.render(x * 16 + 0, y * 16 + 0, 4 + 3 * 32 + icon, col, 0); // renders
																			// the
																			// top-left
																			// part
																			// of
																			// the
																			// tile
		screen.render(x * 16 + 8, y * 16 + 0, 4 + 3 * 32 + icon, col, 0); // renders
																			// the
																			// top-right
																			// part
																			// of
																			// the
																			// tile
		screen.render(x * 16 + 0, y * 16 + 8, 4 + 3 * 32 + icon, col, 1); // renders
																			// the
																			// bottom-left
																			// part
																			// of
																			// the
																			// tile
		screen.render(x * 16 + 8, y * 16 + 8, 4 + 3 * 32 + icon, col, 1); // renders
																			// the
																			// bottom-right
																			// part
																			// of
	...
	public void tick(Level level, int xt, int yt) {
		...
		if (random.nextBoolean() == false)
			return; // if the random boolean is false, then skip the rest of the
					// code

		int age = level.getData(xt, yt); // gets the current age of the tile
		if (age < 50)
			level.setData(xt, yt, age + 1); // if the age of the tile is under
											// 50, then add 1 to the age.
	}
	...
	public boolean interact(Level level, int xt, int yt, Player player, Item item, int attackDir) {
		if (item instanceof ToolItem) { // if the item is a tool...
			ToolItem tool = (ToolItem) item; // converts an Item object into a
												// ToolItem object
			if (tool.type == ToolType.SHOVEL) { // if the type is a shovel
				if (player.payStamina(4 - tool.level)) { // if the player can
															// pay the
															// stamina...
					level.setTile(xt, yt, Tile.dirt, 0); // then set the tile to
															// a dirt tile
					return true;
				}
			}
	...
	public void steppedOn(Level level, int xt, int yt, Entity entity) {
		if (random.nextInt(60) != 0)
			return; // if a random number between 0 and 59 does NOT equal 0,
					// then skip the rest of this code
		if (level.getData(xt, yt) < 2)
			return; // if the age of this tile is less than 2, then skip the
					// rest of this code
	...
	private void harvest(Level level, int x, int y) {
		...
		for (int i = 0; i < count; i++) { // cycles through the count
			level.add(new ItemEntity(new ResourceItem(Resource.seeds, random.nextInt(2)), x * 16 + random.nextInt(10) + 3, y * 16 + random.nextInt(10) + 3)); // adds
																																								// seeds
																																								// to
																																								// the
																																								// world
		...
		if (age == 50) { // if the age is equal to 50 (fully grown) then...
			count = random.nextInt(3) + 2; // count will be anywhere between 2
											// to 4
		} else if (age >= 40) { // if the age is larger or equal to 40 then...
			count = random.nextInt(2) + 1; // count will be anywhere between 1
											// and 2
		}
		for (int i = 0; i < count; i++) { // loops through the count
			level.add(new ItemEntity(new ResourceItem(Resource.wheat, random.nextInt(2)), x * 16 + random.nextInt(10) + 3, y * 16 + random.nextInt(10) + 3)); // adds
																																								// wheat
																																								// to
																																								// the
																																								// world

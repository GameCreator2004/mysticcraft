...
public class PlantableResource extends Resource {
	...
	public PlantableResource(String name, int sprite, int color, Tile targetTile, Tile... sourceTiles) {
		this(name, sprite, color, targetTile, Arrays.asList(sourceTiles)); // assigns
																			// everything
	}
	...
	public boolean interactOn(Tile tile, Level level, int xt, int yt, Player player, int attackDir) {
		if (sourceTiles.contains(tile)) { // if the sourceTiles contains the
											// called tile...
			level.setTile(xt, yt, targetTile, 0); // sets the source tile into
													// the targetTile
			return true;
		}
		return false;
